...
# from .SAp0 import simple_attack
...
class PSOAttacker(BaseAttacker):
    ...
    def run(self, data, obj_id, **attack_opts):
        ...
        #perturb, loss, n_call = simple_attack(data,obj_id,self.predictor,self.loss,attack_opts,self.obs_length,self.pred_length)
        ...
        for k in range(self.attack_duration):
            ...
            best_out[str(k)] = output_data
        #使用Simple perturbation为ready_preturbation使用boundary时是perturb
        return {
            "output_data": best_out, 
            # "perturbation": {obj_id: ready_perturbation_tensor[:self.obs_length].cpu().detach().numpy()},
